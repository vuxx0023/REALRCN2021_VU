...
2. Perform simple arithmetic in RStudio.  
...
## The RStudio Console
Find the carrot `>` within the Console window of RStudio. This is where commands are entered. Because R is a scripting language, if you use the Console within RStudio (or open R itself), you don't point and click to run commands. Instead, you write instructions that are used by the computer to complete an operation. This can make it hard to learn at first because the instructions are specific. R is **not** tolerant of typos, punctuation errors, errant spaces, or other types of mistakes.  

## Arithmetic  
You can use R as a calculator; just enter the expression and press return.  Experiment by entering the following expression within the console.
```{r arithmetic-ex-1}
4*12
```
...
## RMarkdown Basics  
RMarkdown is one of many types of documents that can be created in RStudio and is the one we will use most frequently in the short course.  
