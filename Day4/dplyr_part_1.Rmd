...
```{r}
final_exam_correlation <- select(edu_dat, "gpa_prior",  "final_exam")
...
```{r}
edu_dat %>% select(gpa_prior, final_exam)
```
...
edu_dat %>% 
  ...
  ggplot(aes(gpa_prior, final_exam)) + geom_point()
```
